...
export const BridgeMiniStatus: React.FC<{ state: WorkspaceState }> = ({ state }) => {
  ...
  return (
    <div className="no-scrollbar flex shrink-0 items-center gap-3 overflow-x-auto border-y border-border bg-surface/40 px-3 py-1.5 text-[11.5px] sm:px-4">
      <span className="inline-flex shrink-0 items-center gap-1.5 text-muted-foreground"><Brain className="size-3.5 text-gpt" /><span className={`size-1.5 rounded-full ${chatgpt?.connection_status === 'active' ? 'animate-pulse-dot bg-gpt' : chatgpt?.connection_status === 'offline' || !chatgpt ? 'bg-muted-foreground' : 'bg-gpt/70'}`} />Sol 5.6 {targetStatus(chatgpt)}</span>
      <span className="inline-flex shrink-0 items-center gap-1.5 text-muted-foreground"><Boxes className="size-3.5 text-studio" /><span className={`size-1.5 rounded-full ${studio?.connection_status === 'active' ? 'animate-pulse-dot bg-studio' : studio?.connection_status === 'offline' || !studio ? 'bg-muted-foreground' : 'bg-studio/70'}`} />Gemini 3.8 Flash {targetStatus(studio)}</span>
      {!workspace && <span className="ml-auto text-[10px] text-muted-foreground/70">{state.project?.project_name || 'Bridge'}</span>}
    </div>
  );
};
