...
presence.recordRuntimeHeartbeat({transport:'cli',subject:'blocked-runner',agents:['gemini'],agent_status:{gemini:{state:'blocked',reason:'permission check failed',updated_at:new Date().toISOString()}}},60_000);
assert.equal(presence.runtimeAgentAvailable('gemini'),false,'a runner-reported blocked agent is not advertised as available');
assert.equal(presence.runtimeAgentStatus('gemini').state,'blocked');
