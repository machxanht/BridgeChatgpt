...
import { buildNativeLaunch, NativeTranscript, extractAntigravityAnswer } from '../pc-executor/nativeAdapters.js';
...
assert.throws(()=>parse([lines[0],{event:'result',result:{...lines[1].result,status:'ERROR'}}]).finish(0),/successful|status ERROR/);
...
assert.throws(()=>parse([lines[0],{event:'step_update',step_update:{tool_info:{error:{type:'TOOL_ERROR',message:'permission check failed'}}}},{event:'result',result:{status:'SUCCESS',conversation_id:sessionId,response:'',denied_actions:[{action:'command'}]}}]).finish(0),/permission check failed|denied/,'tool denial is reported as a native failure, never as an empty answer');
