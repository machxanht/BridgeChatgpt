...
function run(model,text){if(model==='gemini'||model==='sonnet'||model==='opus'){const m=model==='gemini'?'gemini-3.8-flash-high':model==='sonnet'?'claude-sonnet-4-6':'claude-opus-4-6-thinking';return execFileSync(agy,['-p',text,'--model',m],{encoding:'utf8',timeout:180000}).trim()}if(model==='codex'||model==='astra')return execFileSync(codex,['exec',text],{encoding:'utf8',timeout:180000}).trim();throw new Error('unsupported '+model)}
async function tick(){const tasks=await req('/api/tasks?limit=300');for(const t of tasks){if(!['pending','assigned'].includes(t.status))continue;const b=binding(t.description);if(!b||!['gemini','sonnet','opus','codex','astra'].includes(b.model))continue;try{await req('/api/tasks/'+t.id,'PATCH',{status:'working',agent:t.assignee});const answer=run(b.model,prompt(t));await req('/api/tasks/'+t.id,'PATCH',{status:'completed',result:answer,agent:t.assignee})}catch(e){console.error(t.id,e.message)}}}
