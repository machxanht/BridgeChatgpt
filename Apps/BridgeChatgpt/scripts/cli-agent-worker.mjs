...
async function tick(){const tasks=await req('/api/tasks?limit=300');for(const t of tasks){if(!['pending','assigned'].includes(t.status))continue;const b=binding(t.description),model=b?.model||b?.mode;if(!b||!['gemini','sonnet','opus','codex','astra'].includes(model))continue;try{const answer=run(model,prompt(t));await req('/api/tasks/'+t.id,'PATCH',{status:'completed',result:answer})}catch(e){console.error(t.id,e.message)}}}
