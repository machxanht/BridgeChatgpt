...
## 2026-09-15 — ChatGPT Standard browser route

Implemented Bridge Wake 2.1.0: authenticated runtime grants now send session CSRF; update preserves enabled state; Standard uses the visible default ChatGPT web selection rather than stale model slugs/cache. Registry/UI label is ChatGPT Standard. Browser account metadata selector is hidden because the Chrome profile owns the actual login. Server rejects old browser workers before a new claim. Native Codex/AGY installation is unchanged.

Validation: browserRuntimeV2 fixtures PASS (one click across disconnects, CSRF grant, default selection, Thinking/unknown rejection); typecheck/build and isolated chatRuntime PASS. No provider generation requested. LIVE STANDARD E2E NOT VERIFIED: only Codex in-app browser is controllable in this session; Chrome/Edge extension is not connected to automation. Operator must load/reload E:\AI\Bridge\Apps\BridgeChatgpt\browser-wake in Chrome/Edge, sign into both sites in the same profile, and enable receiving chats. No admin PowerShell required. README in that folder contains exact steps. Do not advertise Standard as live until browser heartbeat and one matching final receipt are observed.
Deployment evidence: source e1e22ca deployed successfully to Railway as dd282d56-2f7e-4567-8e0d-b303c66350fb. /ready returned ready=true; served asset /assets/index-CS9Xr9-y.js contains ChatGPT Standard and Bridge Wake 2.1.0 instructions. In-app Bridge tab currently shows sign-in; no authenticated live Standard turn was sent.
